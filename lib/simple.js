...
exports.smsg = (ilhansykes, m, hasParent) => {
  if (!m) return m
  let M = m.constructor
	if (m.key) {
		m.id = m.key.id
    m.isBaileys = m.id.startsWith('3EB0') && m.id.length === 12
		...
		m.sender = m.fromMe ? ilhansykes.user.jid : m.participant ? m.participant : m.key.participant ? m.key.participant : m.chat
	}
	if (m.message) {
		m.mtype = Object.keys(m.message)[0]
		m.msg = m.message[m.mtype]
		if (m.mtype === 'ephemeralMessage') {
			exports.smsg(ilhansykes, m.msg)
			m.mtype = m.msg.mtype
			m.msg = m.msg.msg
		}
      ...
      m.quoted.isBaileys = m.quoted.id ? m.quoted.id.startsWith('3EB0') && m.quoted.id.length === 12 : false
		  ...
		  m.quoted.fromMe = m.quoted.sender == ilhansykes.user.jid
			m.quoted.text = m.quoted.text || m.quoted.caption || ''
		  m.quoted.mentionedJid = m.quoted.contextInfo ? m.quoted.contextInfo.mentionedJid : []
			m.getQuotedObj = m.getQuotedMessage = async () => {
        if (!m.quoted.id) return false
        let q = await ilhansykes.loadMessage(m.chat, m.quoted.id)
        return exports.smsg(ilhansykes, q)
      }
      ...
      if (m.quoted.url) m.quoted.download = () => ilhansykes.downloadM(vM)
      m.quoted.copy = () => exports.smsg(ilhansykes, M.fromObject(M.toObject(vM)))
      m.quoted.forward = (jid, forceForward = false)  => ilhansykes.forwardMessage(jid, vM, forceForward)
      m.quoted.copyNForward = (jid, forceForward = false, options = {})  => ilhansykes.copyNForward(jid, vM, forceForward, options)
      m.quoted.cMod = (jid, text = '', sender = m.quoted.sender, options = {}) => ilhansykes.cMod(jid, vM, text, sender, options)
		}
    if (m.msg.url) m.download = () => ilhansykes.downloadM(m)
		m.text = m.msg.text || m.msg.caption || m.msg || ''
    m.reply = (text, chatId, options) => ilhansykes.reply(chatId ? chatId : m.chat, text, m,  options)
    m.copy = () => exports.smsg(ilhansykes, M.fromObject(M.toObject(m)))
    m.forward = (jid, forceForward = false)  => ilhansykes.forwardMessage(jid, m, forceForward)
    m.copyNForward = (jid, forceForward = false, options = {})  => ilhansykes.copyNForward(jid, m, forceForward, options)
    m.cMod = (jid, text = '', sender = m.sender, options = {}) => ilhansykes.cMod(jid, m, text, sender, options)
	}
  return m
}
