...
const { getRandom } = require('./myfunctions')
...
exports.getBuffer = getBuffer = async (url) => {
    ...
    const anu = fs.readFileSync('./ilhansykes.jpeg')
...
exports.createExif = (pack, auth) =>{
    ...
    fs.writeFileSync('./media/data.exif', Buffer.concat([_, __, ___, ____]), function (err) {
        console.log(err)
        if (err) return console.error(err)
        return `./media/data.exif`
    })
...
exports.modStick = (media, client, mek, from) => {
    ...
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './media/data.exif', media, '-o', out])
        .on('exit', () => {
